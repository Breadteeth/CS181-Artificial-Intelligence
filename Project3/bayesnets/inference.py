...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: BayesNet, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for variable in eliminationOrder:
            # use joinFactorsByVariable to join all of the factors that contain a variable in order
            CPTs, joinedFactor = joinFactorsByVariable(CPTs, variable)
            '''
            If a factor that you are about to eliminate a variable from has 
            only one unconditioned variable, you should not eliminate it 
            and instead just discard the factor.-> otherwise, eliminate and append to our new CPTs!
            '''
            if len(joinedFactor.unconditionedVariables()) > 1: CPTs.append(eliminate(joinedFactor, variable))
        return normalize(joinFactors(CPTs)) # debug here---don't forget joinFactors!!!!
        "*** END YOUR CODE HERE ***"
