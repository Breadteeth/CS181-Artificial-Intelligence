...
def constructBayesNet(gameState: GameState):
    ...
    "*** YOUR CODE HERE ***"
    # The full set of observation variables
    # populate `obsVars` using the procedure above
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
    ...
    # populate edges
    edges.extend([(FOOD_HOUSE_VAR, obsVar) for obsVar in obsVars] + [(GHOST_HOUSE_VAR, obsVar) for obsVar in obsVars])
    edges.extend([(X_POS_VAR, house_var) for house_var in HOUSE_VARS] + [(Y_POS_VAR, house_var) for house_var in HOUSE_VARS])

    # set domains
    variableDomainsDict.update({X_POS_VAR: X_POS_VALS, Y_POS_VAR: Y_POS_VALS, FOOD_HOUSE_VAR: HOUSE_VALS, GHOST_HOUSE_VAR: HOUSE_VALS})
...
def fillYCPT(bayesNet: BayesNet, gameState: GameState):
    ...
    "*** YOUR CODE HERE ***"
    # just use setProbability is ok
    # The Y positions are given by values BOTH_TOP_VAL, BOTH_BOTTOM_VAL, LEFT_TOP_VAL, LEFT_BOTTOM_VAL.
...
def getMostLikelyFoodHousePosition(evidence: Dict, bayesNet: BayesNet, eliminationOrder: List[str]):
    ...
    """
    "*** YOUR CODE HERE ***"
    queryVariables = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)
    most_probable_assignment = max(queryVariables.getAllPossibleAssignmentDicts(), key= queryVariables.getProbability) # no () here
