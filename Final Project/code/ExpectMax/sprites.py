...
class BaseSprite(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.direction == Settings.LEFT:
            ...
            self.rect.y += self.speed
        elif self.direction == Settings.HOLD:
            return
        
    def getCoordinate(self):
        x, y = int(self.rect.centerx/50), int(self.rect.centery/50)
        return x, y
...
class Hero(TankSprite):
    ...
    def __init__(self, image_name, screen):
        ...
        self.terminal = 0
        # self.terminal = float(40*random.randint(2, 8))
    ...
    def getLegalAction(self):
        ...
        x, y = self.rect.centerx / Settings.BOX_SIZE - 0.5, self.rect.centery / Settings.BOX_SIZE - 0.5    
        LegalAction = [Settings.HOLD]         
        
        if x % 1 == 0.0 and y % 1 == 0.0: #可以四向移动
            if(int(x) != 0 and Map[int(y)][int(x)-1] != 1 and Map[int(y)][int(x)-1] != 2 and Map[int(y)][int(x)-1] != 5):
                LegalAction.append(Settings.LEFT)
            if(int(x) != 18 and Map[int(y)][int(x)+1] != 1 and Map[int(y)][int(x)+1] != 2 and Map[int(y)][int(x)+1] != 5):
                LegalAction.append(Settings.RIGHT)
            if(int(y) != 0 and Map[int(y)-1][int(x)] != 1 and Map[int(y)-1][int(x)] != 2 and Map[int(y)-1][int(x)] != 5):
                LegalAction.append(Settings.UP)
            if(int(y) != 12 and Map[int(y)+1][int(x)] != 1 and Map[int(y)+1][int(x)] != 2 and Map[int(y)+1][int(x)] != 5):
                LegalAction.append(Settings.DOWN)
        elif x % 1 != 0.0: #只能在x轴上移动
            LegalAction.append(Settings.LEFT)
            LegalAction.append(Settings.RIGHT)
        elif y % 1 != 0.0: #只能在y轴上移动
            LegalAction.append(Settings.UP)
            LegalAction.append(Settings.DOWN)
        return list(set(LegalAction))


    # 我暂时没有使用
    def getAction(self):
        return random.choice(self.getLegalAction())
    ...
    def random_turn(self):
        ...
        self.terminal = float(50)
        # self.terminal = float(50*random.randint(2, 8))
        return directions[random.randint(0, 3)]

    # 每次都攻击
    def random_shot(self):
        shot_flag = random.choice([True])
        if shot_flag:
            super().shot()
    
    def RandomAction(self):  
        if (self.terminal <= 0) or (self.is_hit_wall):
            self.direction = self.random_turn()
        else:
    ...
    def update(self):
        if (not self.is_hit_wall) or (self.terminal > 0):
            self.terminal -= self.speed
            super().update()
            self.__turn()
...
class Wall(BaseSprite):
    ...
    def boom(self): 
        ...
        # Settings.MAP_ONE[int(x)][int(y)] = 0
        pygame.mixer.music.load(Settings.BOOM_MUSIC)
        pygame.mixer.music.play()
        for boom in Settings.BOOMS:
            self.image = pygame.image.load(boom)
