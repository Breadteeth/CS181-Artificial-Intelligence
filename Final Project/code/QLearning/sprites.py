...
class Hero(TankSprite):
    ...
    def getLegalAction(self, state):
        ...
        x, y = round(state.hero.rect.centerx / Settings.BOX_SIZE - 0.5), round(state.hero.rect.centery / Settings.BOX_SIZE - 0.5)    
        LegalAction = []
    ...
    def getAction(self, state):
        if util.flipCoin(self.epsilon):
            return random.choice(self.getLegalAction(state))
        else:
            return self.computeActionFromQValues(state)

    def computeActionFromQValues(self, state):
        legalAction = self.getLegalAction(state)
        if len(legalAction) == 0:
            return None
        else:
            return max(legalAction, key= lambda action: self.getQValue(state, action))

    def getQValue(self, state:AgentState, action:int):
        self.features = getFeatures(state, action)
        return sum([self.weights[oneFeature] * self.features[oneFeature] for oneFeature in self.features])

    def computeValueFromQValues(self, state):
        AllQValues = [self.getQValue(state, action) for action in self.getLegalAction(state)]
        if len(AllQValues) == 0:
            return 0.0
        else:
