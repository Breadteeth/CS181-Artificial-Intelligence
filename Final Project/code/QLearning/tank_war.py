...
class TankWar:
    ...
    def __create_sprite(self):
        self.hero = Hero(Settings.HERO_IMAGE_NAME, self.screen, self)
    ...
    def __AI_agent(self):
        if self.move_cool == 0:
            self.move_cool = 9
            state = AgentState(self.hero, self.enemies, self.enemy_bullets)
            NewDirection = self.hero.getAction(state)
            if NewDirection == Settings.HOLD:
                self.hero.direction = Settings.HOLD
                self.hero.is_moving = False
            else:
                self.hero.direction = NewDirection
                self.hero.is_moving = True
                self.hero.is_hit_wall = False
        else:
            self.move_cool -= 1
        if self.hero.getShoot():
            self.hero.shot()
    ...
    def __update_sprites(self):
        ...
        if self.hero.is_moving:
            self.hero.update(state, random.choice([0,1,2,3]), state, 1)
        self.walls.update()
