...
class TankBattle(object):  # 坦克大战类
    ...
    def __bullets_update(self):
        ...
        for bullet in self.bullets_player:
            is_hit = False
            if self.num_of_enemies - len(self.enemies) == 5:  # 训练时，全场共5个敌人   # 敌人都被消灭
                self.next_rewards_p1.append(500)    #################
                self.total_score_p1 = self.total_score_p1 + 500     ##################
            ...
            for enemy in enemies_hit:
                ...
                if bullet.owner == GlobalConstants.PLAYER_1_OWNER:
                    self.total_score_p1 = self.total_score_p1 + 100    ##################
                    self.next_rewards_p1.append(100)    ##################
                else:
                    self.total_score_p2 = self.total_score_p2 + 100    ##################
                    self.next_rewards_p2.append(100)    ##################
                # self.__generate_enemies(1)
                is_hit = True
                break
            # if num_enemy_hit == 5:   # 训练时，全场共5个敌人
            #     self.next_rewards_p1.append(500)    #################
            #     self.total_score_p1 = self.total_score_p1 + 500     ##################
            #     self.end_of_game = True
            if is_hit:
